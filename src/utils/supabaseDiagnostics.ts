  ...
  checkEnvironmentVariables(): DiagnosticResult[] {
    ...
    if (!supabaseUrl) {
      results.push({
        ...
        details: 'NEXT_PUBLIC_SUPABASE_URL=your-project-url.supabase.co を .env ファイルに設定してください'
      });
    ...
    if (!supabaseAnonKey) {
      results.push({
        ...
        details: 'NEXT_PUBLIC_SUPABASE_ANON_KEY=your-anon-key を .env ファイルに設定してください'
      });
  ...
  async testNetworkConnection(): Promise<DiagnosticResult> {
    ...
    if (!supabaseUrl) {
      return {
        ...
        status: 'error',
        message: 'NEXT_PUBLIC_SUPABASE_URL が設定されていないためネットワークテストを実行できません'
      };
    }
